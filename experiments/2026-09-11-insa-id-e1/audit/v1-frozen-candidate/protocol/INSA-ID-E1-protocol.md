# INSA-ID-E1 — Protocol (frozen pre-execution)

**Status:** v0.1 (frozen-candidate), pending Frank-as-PI GO for execution.
**Author:** Hermes (operator).
**Date:** 2026-09-11.
**Binding:**
- Proposal: `docs/proposals/2026-09-10-INSA-ID-E1-proposal.md` @ `ed95705632027f459b396cd423eae54e8bb9a81b` (proposal v4)
- Frozen v0.3 architecture blob: `848e0fe014f5b4a61ba2cb92e772ee3499dca9c1` (commit `d2c2ad93d95d048e6e2e0c3d42d993a1ecd40f1b`)
- Frozen v0.1 protocol: `experiments/2026-09-06-dbi-evolution-v0.1/protocol/PROTOCOL-v0.5-frozen-final.md` @ `8874692d560d9a6363ef4105fae5384b18cf6ef2` (referenced but NOT modified)
- Frozen BIB source commit: `c369215024c9f8a849daf11bd4b872d7ee566a7a`
- This protocol binds proposal v4 INV-A..INV-APPL-STOP-1 invariants; all 16 frozen pre-execution artifacts are content-addressed in `MANIFEST.json`.

**This protocol is NOT execution authorization.** Execution requires a separate Frank-as-PI GO referencing this protocol SHA, MANIFEST.json SHA, the 16 frozen pre-execution artifact SHAs, the INSA v0.3 blob SHA, and the authority manifest.
...
- The authority manifest (`inputs/authority-manifest.json`)
...
**Arm C (control):** frozen intent (`D` = `inputs/intent-document.txt`) + no-op directive (`inputs/arm-c-directive.txt`). Reconstruction input: `inputs/reconstruction-input.txt` (the BIB baseline at source commit `c3692150`).

**Arm M (modification):** same frozen intent (`D`) + frozen modification specification (`inputs/modification-specification.txt`). Reconstruction input: `inputs/reconstruction-input.txt` + the modification-spec layered on top via a separately delimited second instruction (mirroring v0.1 §3 two-arm construction).
...
**Candidate count per (R, B, arm):** Per v0.1, the OS-CSPRNG draw produces 10 candidate draws per (R, B, arm). See `protocol/EXECUTION-ORDER.md`.
...
| INV-D-1 | `D` (intent document) content-addressed | `preflight/INV-D-1.md` (records `inputs/intent-document.txt` SHA = `7b10cb0641ab68572c7cf9d89a9c49f1afdc3428`) |
| INV-O-1 | `O` (identity contract) content-addressed | `preflight/INV-O-1.md` (records `inputs/identity-contract.txt` SHA = `7ef4356f657884d65dbd4462d85c1c81b3f6fa2a`) |
...
| INV-AUTH-2 | Authority grant authentic + traceable | `preflight/INV-AUTH-2.md` (records grant provenance + Telegram approval message SHA) |
| INV-AUTH-5 | Freshness before consequential commit | `preflight/INV-AUTH-5.md` (records the freshness-witness protocol per I-AUTH-05; per-candidate witnesses produced during execution) |
...
**Execution order:** OS-CSPRNG draw (per v0.1 §5; `protocol/EXECUTION-ORDER.md`) determines the order of (R, B, arm, candidate) execution. The draw is reproducible: the same seed produces the same order.

**Clean-room input delivery:** Each per-(R, B, arm, candidate) execution receives:
- The frozen reconstruction input (`inputs/reconstruction-input.txt`, byte-identical for both arms)
- For Arm M only: the modification specification (`inputs/modification-specification.txt`), layered via a separately delimited second instruction

The executor never sees the other arm's directive.
...
Evaluators A (Codex CLI gpt-5.6-sol) and B (Claude Code CLI claude-opus-4-7, fresh session per I-AUTH-05) receive only the candidate output text + the evaluator input packet. They do NOT see:
...
The evaluator input packet is `evaluation/evaluator-input-packet.md` (frozen at protocol freeze). It contains only:
- The candidate output text
- The BIB behavioral contract excerpt relevant to scoring (no arm identity)
- The acceptance test rubric (for A evaluators) or preservation gate rubric (for G evaluators)
- The blinded output format expected from the evaluator
...
**Modification_Success = A-mod-a AND A-mod-b** (proposal v4 §5.4 STEP 1). A-mod-c and A-mod-d are intra-evaluator stability checks; report-only.

**Non-target_Identity_Preservation = G-pres-subset-a-A AND G-pres-subset-a-B AND G-pres-subset-b-A AND G-pres-subset-b-B AND no per-axis C12-axis-BROKEN** (proposal v4 §5.4 STEP 1).
...
## §8. Control validity pre-check C20 (proposal v4 §5.4 pre-check)

Before any M/P analysis, Arm C must satisfy the frozen BIB non-deviated envelope **on its own** (no +1.5 tolerance added to Arm C; per v0.1 §11.2 PI freeze correction 1, C-1).

C20 fail → STOP → `INVALID_EXPERIMENT` disposition. The report explains which envelope boundary was violated.

The C20 check is computed as: for each (R, B, arm=C) candidate, the BIB 4-dim Manhattan distance from the historical BIB envelope (BIB-001 non-deviated + BIB-002 = 85 observations, per `inputs/baseline-envelope-membership.json`) must lie within the historical envelope. Per-criterion scores must be within the historical BIB calibrated 0-4 range.

---

## §9. Runtime failure handling (proposal v4 §5.5)
...
```
PRE-CHECKS (must all PASS to enter substantive analysis):
  ...
  C20 (Arm C satisfies frozen BIB non-deviated envelope on its own)
  ...
  If any pre-check fails:
     ...
     - Arm C fails own envelope     -> INVALID_EXPERIMENT
...
STEP 1 - Compute per-evaluator (no pooling):
  Modification_Success = A-mod-a AND A-mod-b (binding to M)
  Non-target_Identity_Preservation = G-pres-subset-a-A AND G-pres-subset-a-B
                                     AND G-pres-subset-b-A AND G-pres-subset-b-B
                                     AND no C12-axis-BROKEN (binding to P)

STEP 2 - Apply §9 runtime classification (T1/T2/T3):
  - If systematic (threshold met) -> EXECUTOR_RUNTIME_FAILURE; stop substantive analysis for affected substrate
  - Otherwise proceed to STEP 3 using unaffected candidates

STEP 3 - Substantive classification (joint, not ordered ELSE IF):
...
- Every claim of "modification succeeded at dimension `m`" must cite the frozen `A_m` acceptance test (`inputs/acceptance-tests.json`), the frozen `M` specification (`inputs/mutation-dimensions.json` + `inputs/modification-specification.txt`), the evaluator's recorded M-check outcome, and the aggregated gate result.
- Every authority claim must cite the recorded authority manifest's scope (`inputs/authority-manifest.json`), grant authenticity, and freshness evidence at the M-commit boundary.
...
- `runs/<R>/<B>/<arm>/evidence-<N>.json` — per-candidate evidence (timestamps, model, substrate, freshness witness)
...
- `evaluation/evaluator-A-de-blinding-<candidate>.json` — per-candidate de-blinding key (auditable; cross-referenced to `results/de-blinding-table.json`)
...
- Model identifier (claude-opus-4-7 for executor; gpt-5.6-sol for evaluator A)
...
**C20 (control validity):** If Arm C fails the frozen BIB non-deviated envelope on its own (per §8 above), STOP and classify `INVALID_EXPERIMENT`.
...
This protocol's authorization boundary is protocol + frozen-artifact preparation only. The Frank-as-PI execution GO is a separate event with its own SHA references.

When the execution GO is issued, it must include:
- This protocol's SHA (filled after `MANIFEST.json` is content-addressed)
- `MANIFEST.json` SHA
...
**End of protocol v0.1 (frozen-candidate). Awaiting Frank-as-PI execution GO.**