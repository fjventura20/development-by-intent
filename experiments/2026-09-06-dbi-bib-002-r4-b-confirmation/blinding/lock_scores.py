...
Per EVALUATION-PROCEDURE.md §'Locking scores' + OPERATOR-INSTRUCTIONS.md §4:
  1. complete all 30 scores (already verified by parse step)
...
def lock(evaluator_id: str):
    ...
    if len(scores) != 30:
        raise SystemExit(f"FATAL: {evaluator_id} has {len(scores)} scores, expected 30. Do not lock.")
    blind_ids = [s["blind_id"] for s in scores]
    if len(set(blind_ids)) != 30:
    ...
    print(f"[{evaluator_id}] LOCKED 30 scores")
