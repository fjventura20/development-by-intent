#!/usr/bin/env python3
"""
Build evaluator input packets for DBI-BIB-001-RERUN-001.

Each packet is a single file the evaluator reads top-to-bottom:
  1. frozen behavioral baseline (the contract)
  2. frozen evaluator rubric (the scoring instructions)
  3. 60 candidate records in the order specified by evaluator-X-order.json
     each candidate is:
       ### CANDIDATE <blind_id>
       TEST INPUT: <prompt>
       --- CANDIDATE OUTPUT ---
...
The packet includes NO reconstruction_id, block, test_id, session_id,
model, timestamp, or any other provenance. Only blind_id + test_prompt +
raw output text are visible per candidate.

The packet is written to evaluation/evaluator-A-input.md and
evaluation/evaluator-B-input.md.
"""
import json
...
def build_packet(evaluator_id: str, ordering_path: Path, blind_map_path: Path, contract_path: Path, rubric_path: Path, packet_path: Path):
    ...
    out = []
    out.append(f"# DBI-BIB-001-RERUN-001 — Evaluator {evaluator_id} Input Packet\n")
    out.append(f"**Experiment:** DBI-BIB-001-RERUN-001\n")
    out.append(f"**Evaluator role:** {evaluator_id}\n")
    out.append(f"**Packet generated:** 2026-09-06T09:35:00Z by Hermes (operator)\n")
    out.append("\n---\n")
    out.append("## Instructions to evaluator\n")
    out.append("\n")
    out.append("This packet contains a frozen behavioral contract, a frozen scoring rubric, and exactly **60 candidate outputs** of an application called Amazing Birthday. Each candidate is identified only by a blind ID. You must score every candidate using the rubric below.\n")
    out.append("\n")
    ...
    out.append("3. Section 3 — the 60 candidate records (each: blind ID, exact test input, raw output text)\n")
    out.append("\n")
    out.append("**Materials NOT visible to you (deliberately withheld per EVALUATION-PROCEDURE.md §'Evaluator input packet'):**\n")
    out.append("- which reconstruction produced the candidate (R1–R6 are hidden)\n")
    ...
    out.append("- prior Amazing Birthday outputs or scores\n")
    out.append("- the DbI experimental hypothesis\n")
    out.append("\n")
    out.append("**Your task:**\n")
    out.append("\n")
    out.append("Score **every** candidate independently using the rubric in Section 2. Do NOT compare candidates against one another while scoring. Return one structured JSON record per candidate. After all 60 records are returned, the operator will lock your score set, hash it, and only then unblind.\n")
    out.append("\n")
    ...
    out.append(rubric)
    out.append("\n---\n\n")
    out.append("## Section 3 — 60 Candidates (in randomized order; do NOT compare against one another)\n\n")

    for i, bid in enumerate(ordered_ids, 1):
        c = mapping[bid]
        out.append(f"### CANDIDATE {i:02d} of 60 — blind_id `{bid}`\n\n")
    ...
    out.append("Return your output as a single fenced JSON code block containing a JSON array of 60 records, in the **same order** as the candidates above (CANDIDATE 01 ... CANDIDATE 60). Each record must include:\n\n")
    out.append("- `blind_id`\n")
...
def main():
    ...
    rubric = INPUTS.parent  # rubric lives in the execution-package freeze — copy into inputs/
    # We'll copy the rubric into inputs/ for evaluator visibility
    rubric_dst = INPUTS / "EVALUATOR-RUBRIC.md"
    if not rubric_dst.exists():
        import shutil
        # fetch from the frozen commit
        import subprocess
        result = subprocess.run(
            ["git", "-C", str(EVDIR.parent.parent), "show", "00676a3:experiments/behavioral-identity-baseline-v0.1/execution-package-v0.1/EVALUATOR-RUBRIC.md"],
            capture_output=True, check=True,
        )
        rubric_dst.write_bytes(result.stdout)

    blind_map = BLINDING / "blind-map.json"

    a_size = build_packet(
        "A",
        BLINDING / "evaluator-A-order.json",
        blind_map,
        contract,
        rubric_dst,
        EVAL / "evaluator-A-input.md",
    )
    print(f"Wrote {EVAL / 'evaluator-A-input.md'} ({a_size} bytes)")

    b_size = build_packet(
        "B",
        BLINDING / "evaluator-B-order.json",
        blind_map,
        contract,
        rubric_dst,
        EVAL / "evaluator-B-input.md",
    )
