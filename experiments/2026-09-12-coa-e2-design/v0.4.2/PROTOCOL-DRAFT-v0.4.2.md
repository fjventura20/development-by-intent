# COA-E2 v0.4.2 — Persistent Session Binding Proof of Concept

**Status:** targeted draft only; not frozen and not executable. This v0.4.2 package
is a narrowly scoped successor to the v0.4.1 protocol-compliant re-freeze
(freeze record id `20260914T062248Z-coa-e2-v0.4.1-refreeze-001`, commit
`805153e`). It corrects exactly three execution-blocking defects identified
in the final pre-execution implementation audit:

1. blank-state hash computation bug in `execute_pair()`,
2. missing sealed-expected runtime identity binding,
3. six-nonce semantics: only `*-init` nonces materially participated.
...
unchanged as historical evidence. v0.4.2 does not amend, rewrite, or
delete any of them.
