...
def dry_validate(package: Path, manifest_path: Path) -> dict:
        ...
        if m.get("status") == "SEALED":
            ...
            for k in ("hermes_version", "model", "provider",
                ...
                if v in (None, "", "<TO_BE_SEALED>"):
                    errors.append(
                        f"sealed expected_runtime_identity.{k} is not sealed "
                        f"(value={v!r})")
    ...
    # v0.4.2: rubric filename version bump.
    if not (package / "tasks/RUBRIC-DRAFT-v0.4.2.md").is_file():
        errors.append("rubric missing (expected tasks/RUBRIC-DRAFT-v0.4.2.md)")
    for t in ("U2", "U3", "U5"):
    ...
    result = {
        ...
        "task_prompt_rubric_separation": (
            ...
            != (package / "tasks/RUBRIC-DRAFT-v0.4.2.md").read_text()
        ),
