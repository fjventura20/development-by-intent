# COA-E2 v0.4.2 Deviations and Global STOP Rules
...
| `S11_RUNTIME_IDENTITY`    | Sealed expected runtime identity block is missing or a captured field (hermes_version, model, provider, normalized_config_hash, expected_blank_memory_md_hash, expected_blank_user_md_hash) is missing or mismatches the sealed expected value; or cross-arm equivalence fails. v0.4.2 unifies the prior `S1_RUNTIME_IDENTITY` cross-arm check into this code. |
