...
def dry_validate(package: Path, manifest_path: Path) -> dict:
    ...
    # v0.4.4: rubric filename version bump.
    if not (package / "tasks/RUBRIC-DRAFT-v0.4.4.md").is_file():
        errors.append("rubric missing (expected tasks/RUBRIC-DRAFT-v0.4.4.md)")
    for t in ("U2", "U3", "U5"):
    ...
    fixture_results = {
        ...
        # v0.4.4: targeted Hermes-native state.db bootstrap-and-scrub
        # fixtures.
        "v044_bootstrap_native_schema_scrubbed_zero_counts": _dry_v044_bootstrap_scrubbed_fixture(),
        "v044_bootstrap_residual_rows_FAIL": _dry_v044_residual_rows_fixture(),
        "v044_bootstrap_incomplete_schema_FAIL": _dry_v044_incomplete_schema_fixture(),
    }
    result = {
        ...
        "task_prompt_rubric_separation": (
            ...
            != (package / "tasks/RUBRIC-DRAFT-v0.4.4.md").read_text()
        ),
...
# v0.4.4 Hermes-native state.db bootstrap-and-scrub fixtures.
#
# The fixtures exercise the v0.4.4 preflight invariants by stubbing
# the sqlite3.connect call. They never invoke the real Hermes CLI
# during dry-run; that is the operator's responsibility during the
# execution-preparation phase. The fixtures therefore validate the
# *logic* of the preflight invariant checks (a) on a synthetic
# post-scrub state.db, (b) on a residual-rows case, and (c) on an
# incomplete-schema case.


def _dry_v044_bootstrap_scrubbed_fixture() -> str:
    """Genuine Hermes-created schema, bootstrap rows scrubbed, all
    conversational counts zero, schema fingerprint matches.

    Simulates a successful bootstrap-and-scrub by stubbing
    sqlite3.connect to return an in-memory SQLite database with a
    realistic Hermes schema (the same 24-table layout observed
    empirically on this host). The fixture then runs the same
    preflight invariant checks the runner uses at execute_pair
    time and asserts they all pass.
    """
    try:
        ...
        # Build an in-memory SQLite that mirrors the empirical schema.
        db = _sqlite3.connect(":memory:")
        db.executescript("""
        CREATE TABLE schema_version (version INTEGER);
        INSERT INTO schema_version VALUES (30);
        CREATE TABLE state_meta (key TEXT PRIMARY KEY, value TEXT);
        INSERT INTO state_meta VALUES ('cli_version', 'v0.21.2');
        CREATE TABLE sessions (
            id TEXT PRIMARY KEY, source TEXT, user_id TEXT,
            session_key TEXT, model TEXT, parent_session_id TEXT,
        ...
        CREATE TABLE conversation_generations (
            source TEXT, session_key TEXT, generation INTEGER
        );
        CREATE TABLE session_turn_leases (
            conversation_id TEXT, holder TEXT
        );
        CREATE TABLE compression_locks (
            session_id TEXT, holder TEXT
        );
        CREATE TABLE async_delegations (delegation_id TEXT);
        CREATE TABLE messages_fts (
            content TEXT, tool_name TEXT, tool_calls TEXT
        );
        CREATE TABLE messages_fts_config (k TEXT, v TEXT);
        INSERT INTO messages_fts_config VALUES ('tokenize', 'porter');
        CREATE TABLE messages_fts_data (id INTEGER PRIMARY KEY, block BLOB);
        CREATE TABLE messages_fts_docsize (id INTEGER PRIMARY KEY, sz INTEGER);
        CREATE TABLE messages_fts_idx (segid INTEGER, term TEXT, pgno INTEGER);
        CREATE TABLE messages_fts_trigram (content TEXT, tool_name TEXT);
        CREATE TABLE messages_fts_trigram_config (k TEXT, v TEXT);
        INSERT INTO messages_fts_trigram_config VALUES ('tokenize','trigram');
        CREATE TABLE messages_fts_trigram_data (id INTEGER PRIMARY KEY, block BLOB);
        CREATE TABLE messages_fts_trigram_docsize (id INTEGER PRIMARY KEY, sz INTEGER);
        CREATE TABLE messages_fts_trigram_idx (segid INTEGER, term TEXT, pgno INTEGER);
        CREATE TABLE gateway_routing (scope TEXT, session_key TEXT,
            entry_json TEXT, updated_at TEXT);
        CREATE TABLE gateway_hygiene_state (session_key TEXT, failure_streak INTEGER);
        CREATE TABLE gateway_heartbeats (backend_id TEXT, pid INTEGER,
            started_at TEXT, last_heartbeat TEXT, profile TEXT, host TEXT);
        """)

        # Schema fingerprint (post-scrub) — must be deterministic and
        ...
        def _fp(c):
            ...
            return _hashlib.sha256(
                _json.dumps(inv, sort_keys=True,
                            separators=(',', ':')).encode()
            ).hexdigest()

        captured_fp = _fp(db)

        # Verify all conversational counts are zero (post-scrub state).
        for tbl in ("sessions", "messages", "session_model_usage",
                    "system_prompts", "conversation_generations",
                    "session_turn_leases", "compression_locks",
                    "async_delegations"):
            n = db.execute(f'select count(*) from "{tbl}"').fetchone()[0]
            if n != 0:
                return f"FAIL: {tbl} has {n} rows post-scrub (expected 0)"

        # Verify infrastructure rows are preserved.
        sv = db.execute('select version from schema_version').fetchone()
        if not sv or sv[0] != 30:
            return f"FAIL: schema_version not preserved (got {sv})"
        sm = db.execute("select count(*) from state_meta").fetchone()[0]
        if sm != 1:
            return f"FAIL: state_meta has {sm} rows (expected 1 preserved)"

        # Verify schema fingerprint is non-empty and deterministic.
        recomputed_fp = _fp(db)
        if recomputed_fp != captured_fp:
            return f"FAIL: fingerprint drift {captured_fp} != {recomputed_fp}"
        if len(captured_fp) != 64:
            return f"FAIL: fingerprint wrong length ({len(captured_fp)})"

        # Verify the file-openable invariant (in-memory DBs pass; the
        # real check is sqlite3.connect(path) returning without error,
        # which an in-memory stub also satisfies).
...
def _dry_v044_residual_rows_fixture() -> str:
    """Genuine Hermes-created schema but bootstrap rows were NOT
    scrubbed. The preflight must FAIL.

    Simulates a profile whose state.db contains the bootstrap session
    and message rows. The runner must catch this and refuse.
    """
    try:
        import sqlite3 as _sqlite3
        db = _sqlite3.connect(":memory:")
        ...
        CREATE TABLE session_model_usage (session_id TEXT);
        CREATE TABLE system_prompts (hash TEXT);
        CREATE TABLE conversation_generations (source TEXT, session_key TEXT);
        CREATE TABLE session_turn_leases (conversation_id TEXT);
        CREATE TABLE compression_locks (session_id TEXT);
        CREATE TABLE async_delegations (delegation_id TEXT);
        """)
        # Bootstrap left one session + two messages + one usage row.
        db.execute("insert into sessions values ('bootstrap-sid', NULL)")
        db.execute("insert into messages (session_id, role) values ('bootstrap-sid', 'user')")
        db.execute("insert into messages (session_id, role) values ('bootstrap-sid', 'assistant')")
        db.execute("insert into session_model_usage values ('bootstrap-sid')")
        db.execute("insert into system_prompts values ('h1')")
        db.execute("insert into conversation_generations values ('x','y')")
        db.execute("insert into session_turn_leases values ('c1')")
        db.execute("insert into compression_locks values ('s1')")
        db.execute("insert into async_delegations values ('d1')")

        # Apply the same preflight invariant checks. The runner must
        # detect non-zero counts.
        failures = []
        for tbl in ("sessions", "messages", "session_model_usage",
                    "system_prompts", "conversation_generations",
                    "session_turn_leases", "compression_locks",
            ...
            n = db.execute(f'select count(*) from "{tbl}"').fetchone()[0]
            if n != 0:
                failures.append(f"{tbl}={n}")

        if not failures:
            return "FAIL: residual rows not detected (should fail preflight)"
        # Check that child-session detection works (parent_session_id
        # on a row would imply a forked session).
        # In this fixture the row is a normal session (parent NULL), so
        # we count it as a leftover-bootstrap-session, not a child.
        # The fixture asserts the residual is detected via the count
        # check, which it is.
        return "PASS"
    except Exception as exc:
        return f"FAIL: {type(exc).__name__}: {exc}"
...
def _dry_v044_incomplete_schema_fixture() -> str:
    """Incomplete or incompatible state.db schema must FAIL preflight.

    Simulates the v0.4.3 broken case: hand-crafted partial schema
    with only sessions + messages tables. The runner must detect
    that the schema is missing required infrastructure tables.
    """
    try:
        import sqlite3 as _sqlite3
        db = _sqlite3.connect(":memory:")
        # v0.4.3 hand-crafted schema: only sessions + messages.
        db.executescript("""
        CREATE TABLE sessions (
            id TEXT PRIMARY KEY,
            profile_name TEXT,
        ...
        CREATE TABLE messages (
            ...
            tool_name TEXT, tool_call_id TEXT, created_at TEXT
        );
        """)
        ...
        REQUIRED_TABLES = {
            ...
            "session_model_usage", "system_prompts",
            "conversation_generations",
            "messages_fts_config", "messages_fts_trigram_config",
        }
        missing = REQUIRED_TABLES - tables
        if not missing:
            return "FAIL: incomplete schema not detected"
        return "PASS"
    except Exception as exc:
        return f"FAIL: {type(exc).__name__}: {exc}"
