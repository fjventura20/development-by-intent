...
class PrivilegedExecutor:
    ...
    def _execute_transaction(
        ...
        try:
            ...
            # Apply mutation
            new_count = int(res["mutation_count"]) + 1
            new_state_json = _apply_mutation(res["state_json"], payload["operation"], request["request_id"])
            conn.execute(
                "UPDATE protected_resources SET state_json=?, mutation_count=?, updated_at=? "
                "WHERE resource_id=?",
                (new_state_json, new_count, _iso(self.config.now_utc_fn()), resource_id),
            )
