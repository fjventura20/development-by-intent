#!/usr/bin/env python3
"""Successor formal runner for Agent Conformance Local Lifecycle PoC v0.1.
...
IMPLEMENTATION_BASELINE = "cfd85e809a541a136b478fa7605a7cf4ca798a2d"
...
def main() -> int:
    ...
    try:
        ...
        write_json(evidence_dir / "09_final_classification.json", {
            ...
            "pytest": "68 passed / 0 failed / 0 skipped / 0 xfailed",
        ...
        # Manifest excludes itself and is written last among evidence content.
        manifest_files = []
        for p in sorted(evidence_dir.iterdir()):
            if p.name in {"evidence-manifest.json", "evidence-manifest.sha256"}:
                continue
        ...
        (evidence_dir / "evidence-manifest.sha256").write_text(
