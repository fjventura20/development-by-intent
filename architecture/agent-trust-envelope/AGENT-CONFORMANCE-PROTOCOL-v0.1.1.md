# Agent Conformance Protocol v0.1.1
...
**Supersedes for current design work:** `AGENT-CONFORMANCE-PROTOCOL-v0.1.md`  
**Review incorporated:** `AGENT-CONFORMANCE-PROTOCOL-v0.1-ADVERSARIAL-REVIEW.md`  
...
Conformance Evaluation
    ↓
CONFORMANT / REATTESTATION_REQUIRED / RESTRICTED / SUSPENDED / REVOKED
    ↓
...
No conformance artifact substitutes for qualification, admission, action authorization, revocation state, or execution enforcement.
...
### REVOKED
Affected admission/role lifecycle instance is terminated.
...
Critical revocation, trust-root compromise, or identity invalidation MUST NOT wait for periodic review.
...
R14 does not automatically inherit qualification, admission, trust-root, policy-authoring, or Capability Executor authority.
...
10. Verify trigger-source freshness.
11. Verify bounded conformance freshness.
12. Evaluate scope/partition dependencies.
13. Produce signed evaluation result.
14. Submit result to authorized lifecycle-state authority.
15. Audit evaluation.
...
Enforcement observes new state/epoch
    ↓
Restore / restrict / remain suspended / revoke
```
...
REVOKED is terminal for the affected admission/role lifecycle instance unless superior policy defines explicit reinstatement.

A revoked subject may later requalify and be readmitted as a new lifecycle instance.
...
2. critical suspension/revocation state is checked again inside or immediately before the execution boundary;
...
Every authority-changing transition must preserve:
...
Transitions require authorized R14 decisions except immediate externally authoritative revocation states already defined by controlling trust-state policy, which MUST be honored directly and cannot be overridden by R14.
...
- **P6 Current-state enforcement** — stale positive state cannot override newer suspension/revocation.
