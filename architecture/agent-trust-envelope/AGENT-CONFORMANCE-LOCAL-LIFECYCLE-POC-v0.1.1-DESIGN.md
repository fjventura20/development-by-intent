# Agent Conformance Local Lifecycle PoC v0.1.1 — Design
...
**Reviews incorporated:** `AGENT-CONFORMANCE-LOCAL-LIFECYCLE-POC-v0.1-ADVERSARIAL-REVIEW.md`  
...
2. **Qualification/Admission Fixture**
   - deterministic signed qualification fixture with stable ID/digest
   - deterministic signed admission fixture with stable ID/digest
   - authoritative current-state registry marks both ACTIVE
   - authorization service verifies both fixtures and their current state on every relevant authorization decision
   - no dynamic requalification/readmission in the base happy path

3. **Trigger Observer**
   - reads observer-authoritative measured runtime state
   - measured state is distinct from subject-requested state
   - subject-facing interface cannot rewrite observer history
   - emits signed/deterministically authenticated trigger observation when measured state changes
...
For the PoC, separation may use distinct local keys/files/process identities rather than separate machines. The runtime observer's authoritative measurement store MUST be distinct from any subject-requested runtime input and inaccessible to subject mutation through the tested interface.
...
Before mutation, the active ConformanceRequirementsProfile permits only `v1`. A signed successor profile permitting `v2` MUST already exist and its digest MUST be fixed before formal-run start; the formal run may activate it later but may not invent or alter it after observing the invalidation.
...
## 9. Predeclared Conformance Profiles

Two profiles are frozen before formal-run start:
...
Profile v2 MUST be signed, hashed, and present before the formal run begins. Its activation is a predeclared policy transition, not an after-the-fact weakening to fit observed runtime state.
...
```text
qualification = ACTIVE (verified fixture ID/digest)
admission = ACTIVE (verified fixture ID/digest)
runtime_version = v1
...
## 10A. Qualification/Admission Fixture Verification

The PoC may use static qualification/admission fixtures, but it may not bypass their verification.
...
At every governed capability issuance, the Authorization Service MUST verify:

```text
fixture integrity
AND issuer/authenticator validity
AND subject/role/domain binding
AND current_state == ACTIVE
```

Static fixture state reduces scope; it does not remove the verification gate.
...
## 12A. Exact Executor Ordering

For every execution attempt, the executor MUST perform:

```text
1. verify capability signature
2. verify subject/role/action binding
3. verify capability expiry
4. resolve current qualification/admission state
5. resolve current conformance state and state_epoch
6. reject if state is insufficient
7. reject if capability.observed_state_epoch != current state_epoch
8. atomically reserve nonce
9. obtain protected-resource authority
10. execute exact action
11. record outcome
12. mark nonce terminal
```

Nonce reservation MUST NOT occur before stale/non-conformant rejection. This keeps denial evidence unambiguous and prevents stale capability checks from consuming execution state.
...
The subject cannot satisfy the test by self-reporting this change. The observer reads the authoritative measured-runtime store, and the formal test must demonstrate that subject-facing mutation APIs cannot rewrite prior observer evidence.
...
The PoC next activates the predeclared, already-signed profile v2 so that v2 becomes permitted.

This MUST be represented as activation of the preexisting signed ConformanceRequirementsProfile v2, not an in-place edit and not a profile created after the runtime mutation.
...
The successor profile MUST have been signed/authorized and digest-locked before formal-run start. Activation through the PoC policy fixture occurs only after the invalidation proof is complete.
...
Expected: previously frozen/signed successor profile becomes current; its digest matches the preflight-locked value and requires runtime v2.
...
Expected: sequence contiguous, hashes valid, event ordering valid, and monotonic causal order proves:
```text
C1_issued
< runtime_mutation
< trigger_observed
...
Expected: verification failure against a copied tampered ledger while the authoritative ledger remains unchanged.
...
audit_ledger.jsonl
audit_ledger_tampered_copy.jsonl
audit_verification.json
causal_order_verification.json
test_results.json
...
## 20A. Causal Ordering and Audit Tamper Discipline

Every security-relevant artifact and audit event MUST carry a monotonic `event_sequence` or equivalent ordering field in addition to any wall-clock timestamp.

The formal verifier MUST prove the required causal chain, not merely compare timestamps.

Audit tamper testing MUST be non-destructive:

1. preserve authoritative `audit_ledger.jsonl`;
2. copy it to `audit_ledger_tampered_copy.jsonl`;
3. mutate only the copy;
4. verify the copy fails integrity checks;
5. verify the authoritative ledger still passes.

The test harness MUST NOT repair or regenerate the authoritative ledger after tamper testing.
...
5. stale capability is not independently checked at executor boundary using the required executor sequence;
6. monotonic record ordering cannot prove the required causal sequence;
...
11. profile v2 is generated or modified after formal-run start;
12. qualification/admission current-state verification is bypassed or hard-coded.
...
**Current status:** FINAL FREEZE CANDIDATE — READY FOR NARROW FREEZE REVIEW
